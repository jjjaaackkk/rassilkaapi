...
from core.models import Campaign, Customer, MSG
from core.stats import get_stats_from_camps, get_stats_from_msgs
from .specs import (
    campaign_par,
...
class CustomerAPI1(APIView):
    ...
    @swagger_auto_schema(
        responses=customer_get_response_all,
        operation_description="Запросить список рассылок",
    )
    def get(self, req):
        return Response({"customers": Customer.objects.all().values()})

    @swagger_auto_schema(
        request_body=customer_post_request,
...
class CustomerAPI2(APIView):
    ...
    @swagger_auto_schema(
        responses=customer_get_response, operation_description="Запросить клиента по ID"
    )
    def get(self, req, pk):
    ...
    @swagger_auto_schema(
        responses=customer_put_responses,
...
class CampaignAPI1(APIView):
    ...
    @swagger_auto_schema(
        responses=campaign_get_response_all,
        operation_description="Запросить список рассылок",
    )
    def get(self, req):
        return Response({"campaigns": Campaign.objects.all().values()})

    @swagger_auto_schema(
        request_body=campaign_post_request,
...
class CampaignAPI2(APIView):
    ...
    @swagger_auto_schema(
        responses=campaign_get_response,
    ...
    @swagger_auto_schema(
        request_body=campaign_put_request,
...
class MessageAPI(APIView):
    ...
    @swagger_auto_schema(
        responses=messages_delete_responses,
...
class StatsAPI1(APIView):
    ...
    @swagger_auto_schema(
        responses=stats_get_response_main,
        ...
        return Response(
            {
                "campaign_id": pk,
                "status": status,
                "messages": {"succeed": succeed, "failed": failed},
            }
        )
